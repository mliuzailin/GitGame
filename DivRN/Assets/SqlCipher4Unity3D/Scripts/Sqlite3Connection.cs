...
namespace SqlCipher4Unity3D
{
    ...
    public partial class SQLiteConnection : IDisposable
    {
        ...
        public SQLiteConnection(string databasePath, string password, SQLiteOpenFlags openFlags, bool storeDateTimeAsTicks = false)
        {
...
#if BUILD_TYPE_DEBUG
                //Debug.Log("password: " + password + " / password.Length: " + password.Length);
                //Debug.Log("GetErrmsg: " + SQLite3.GetErrmsg(handle));
#endif
            ...
            /* 
			var rconf = SQLite3.Config(SQLite3.ConfigOption.SingleThread);
			if (rconf != SQLite3.Result.OK) {
				Debug.LogError ("Sqlite3: "+ String.Format ("Could not Config: {0} ({1})", DatabasePath, rconf));
			}
			*/
        ...
        public void initalizePragma()
        {
            //ExecutePragma("PRAGMA integrity_check");

            // http://devlights.hatenablog.com/entry/2014/02/01/151642
            // https://sqlite.org/wal.html
            ExecutePragma("PRAGMA journal_mode", "WAL");
            // https://sqlite.org/pragma.html#pragma_synchronous
            // NORMAL でもInsertOrUpdateが重いのであまり変わらない。安全のためにFULL
            ExecutePragma("PRAGMA main.synchronous", "FULL");
        }

        public bool brokenDb()
        {
            string ret = ExecutePragma("PRAGMA integrity_check");
...
#if BUILD_TYPE_DEBUG
                Debug.Log("Sqlite3: " + string.Format("Finished in {0} ms ({1:0.0} s total)", _sw.ElapsedMilliseconds, _elapsedMilliseconds / 1000.0));
#endif
            }
        ...
        public string ExecutePragma(string query, string param = null)
        {
            string command;
            if (param == null)
            {
...
#if BUILD_TYPE_DEBUG
            Debug.Log("Sqlite3: set: " + command);
#endif
            var cmd = CreateCommand(command);
            var ret = cmd.ExecuteScalar<String>();

#if BUILD_TYPE_DEBUG
            cmd = CreateCommand(query);
            Debug.Log("Sqlite3: ret: " + query + ":" + cmd.ExecuteScalar<String>());
#endif
            return ret;
        }
        ...
        {
            //BeginTransaction →　Commitと速度は変わらなそう
            try
            {
    ...
    public class TableMapping
    {
        ...
        public string TableName
        {
            get
            {
                return DivRNUtil.GetTableName(tableName);
            }
            private set
            {
                tableName = value;
    ...
    public partial class SQLiteCommand
    {
        ...
        public int ExecuteNonQuery()
        {
            ...
            if (_conn.Trace)
            {
                Debug.Log("Sqlite3: Executing: " + this);
            }
#endif
        ...
        {
            TableMapping mapping = _conn.GetMapping(typeof(T));
            IEnumerable<T> query = ExecuteDeferredQuery<T>(mapping);
        ...
        public SimpleDataTable ExecuteQueryGeneric()
        {
            ...
            if (_conn.Trace)
            {
                Debug.Log("Sqlite3: Executing Query: " + this);
            }
#endif
            ...
            for (int c = 0; c < columnCount; c++)
            {
                ...
                {
                    name = SQLite3.ColumnName16(stmt, c)
                });
            }
        ...
        internal static int BindParameter(Sqlite3Statement stmt, int index, object value, bool storeDateTimeAsTicks)
        {
            ...
            if (value is DateTime)
            {
                ...
                {
                    return SQLite3.BindText(stmt, index, ((DateTime)value).ToString("yyyy-MM-dd HH:mm:ss"), -1, NegativePointer);
                }
            }
    ...
    public class PreparedSqlLiteInsertCommand : IDisposable
    {
        ...
        public int ExecuteNonQuery(object[] source)
        {
            ...
            if (Connection.Trace)
            {
                Debug.Log("Sqlite3: Executing: " + CommandText);
            }
#endif
            var r = SQLite3.Result.OK;
