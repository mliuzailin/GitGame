...
namespace AsPerSpec
{
    ...
    public class CarouselToggler : MonoBehaviour, IInitializePotentialDragHandler, IEndDragHandler
    {
        ...
        public UnityEventBool OnChangedSnapCarousel;
        ...
        public void SnapToClosest()
        {
            ...
            if (toggles.Length > 0)
            {
                ...
                bool isChangeToggle = false;
                if (targetToggle == null)
                {
                    isChangeToggle = (toggles[0] != toggleClosest);

                }
                else
                {
                    isChangeToggle = (targetToggle != toggleClosest);
                }

                OnChangedSnapCarousel.Invoke(isChangeToggle);
        ...
        Toggle findClosestToggle(Toggle[] toggles)
        { // closest to scrollRect center
            ...
            foreach (Toggle toggle in toggles)
            {
                ...
                {
                    if (!(nonStick && toggle.isOn))
                    { // if nonStick and is last, skip.
                        minDiff = diff;
            ...
            if (nonStick || stickyFactor < 1)
            {
                ...
                { // we ignored last only because of nonstick
                    ...
                    if (togglesDiff.magnitude < minDistance)
                    { // dragged outwards and last was edge
