...
namespace UniExtensions.Serialization
{
	public class JsonSerializer
	{
		...
		protected static object ParseNumber (char[] json, ref int index, ref bool success)
		{
			...
			if (token.Contains (".")) {
				...
				int number;
				if(int.TryParse(token, NumberStyles.Any, CultureInfo.InvariantCulture, out number)) {
					return (int)number;
				} else {
					return (string)token;
				}
		...
		protected static bool SerializeValue (object value, StringBuilder builder)
		{
			...
			if (value is string) {
                ...
                success = SerializeNumber(Convert.ToInt32(value), builder);
            } else {
				var h = new Hashtable ();
