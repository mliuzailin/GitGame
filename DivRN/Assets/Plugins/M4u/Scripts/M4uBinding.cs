...
namespace M4u
{
	...
	public class M4uBinding : MonoBehaviour
	{
		...
		{
			if (root == null || (root != null && root.Context == null))
			{
				root = GetRoot();
			}
			
#if BUILD_TYPE_DEBUG
			if (root == null) Debug.Log("Start Error:" + this.name + ":root not found!!");
			if (root != null && root.Context == null) Debug.Log("Start Error:" + this.name + ":root Context not found!!");
#endif
