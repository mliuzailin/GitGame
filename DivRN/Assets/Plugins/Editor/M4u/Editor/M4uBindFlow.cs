...
namespace M4u
{
    ...
    public class M4uBindFlow : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            searchString = EditorGUILayout.TextField(searchString, GUI.skin.FindStyle("ToolbarSeachTextField"), GUILayout.MinWidth(200));
                        ...
                        if (!hasRoot || ((M4uBinding)components[0]).GetRoot() == root)
                        {
                            ...
                            func = (_) =>
                            {
                                ...
                                foreach (var component in components)
                                {
                                    ...
                                    {
                                        // []の中の文字を抽出する
                                        string searchStr = component.ToString();
                                        int cutindex = searchStr.IndexOf("=") + 1;
                                        searchStr = searchStr.Substring(cutindex, searchStr.Length - cutindex);
                                        cutindex = searchStr.LastIndexOf("]");
                                        if (cutindex >= 0)
                                        {
                                            searchStr = searchStr.Substring(0, cutindex + 1);
                                        }
                                        searchStr = searchStr.Replace("][", ",");
                                        searchStr = searchStr.Replace("[", "");
                                        searchStr = searchStr.Replace("]", "");
                                        string[] searchStrArray = searchStr.Split(','); // 抽出した文字

                                        int index = Array.FindIndex(searchStrArray, (v) => v.ToLower().Contains(searchString.ToLower()));
                                        if (index >= 0)
                                        {
