...
static NSString *UniWebViewHomeDomain = nil;
static NSString *UniWebViewInBrowserUrls = nil;
...
- (BOOL)isExternalBrowser:(NSURL *)url {
        //URLManager.CheckLoadURL合わせ
        if(UniWebViewHomeDomain != nil)
        {
            if( [url.scheme rangeOfString:@"https"].location == NSNotFound){
                return YES;
            }

            if([url.host rangeOfString:UniWebViewHomeDomain].location == NSNotFound){
                return YES;
            }
        }
        
        if(UniWebViewInBrowserUrls != nil)
        {
            NSArray* values = [UniWebViewInBrowserUrls componentsSeparatedByString:@","];
            for (NSInteger i = 0; i < values.count; i++)
            {
                NSString* value = [values objectAtIndex:i];
                //NSLog(@"UniWebViewInBrowserUrls[%@]: %@",@(i), value);

                if([url.absoluteString rangeOfString:value].location != NSNotFound){
                    return YES;
                }
            }
        }
...
-(BOOL)webView:(UniWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    ...
    if (canResponse) {
        ...
        if (navigationType == UIWebViewNavigationTypeLinkClicked && [self isExternalBrowser:request.URL]) {
            [[UIApplication sharedApplication] openURL:request.URL];
            return NO;
        } else if (navigationType == UIWebViewNavigationTypeLinkClicked && webView.openLinkInExternalBrowser) {
            [[UIApplication sharedApplication] openURL:request.URL];
            return NO;
        } else {
...
extern "C" {
    ...
    void _UniWebViewSetHomeDomain(const char *text);
    void _UniWebViewSetInBrowserUrls(const char *text);
}
...
void _UniWebViewSetHomeDomain(const char *text) {
    NSString *title = UniWebViewMakeNSString(text);
    if (title.length == 0) {
        UniWebViewHomeDomain = nil;
    } else {
        UniWebViewHomeDomain = title;
    }
}
...
void _UniWebViewSetInBrowserUrls(const char *text) {
    NSString *title = UniWebViewMakeNSString(text);
    if (title.length == 0) {
        UniWebViewInBrowserUrls = nil;
    } else {
        UniWebViewInBrowserUrls = title;
    }
}